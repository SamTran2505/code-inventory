...
class RealTimeAdvisor:
    def __init__(self, Q_total_initial, m, M, a=80, b=1, delta=0.2):
...
def run_smart_tool_iphone():
    ...
    try:
        print("\n--- CẤU HÌNH ĐẦU MÙA VỤ ---")
        Q_init = float(input("1. Tổng nhập hàng ban đầu (VD: 1000 máy): "))
        m = float(input("2. Giá sàn/Giá gốc (VD: 30.0 tr): "))
        M = float(input("3. Giá trần kỳ vọng (VD: 40.0 tr): "))
        
        # Tự động set tham số a, b cho iPhone nếu người dùng không rành
        advisor = RealTimeAdvisor(Q_init, m, M, a=80, b=1)
        print("-> Đã thiết lập mô hình cầu chuẩn cho iPhone (a=80, b=1)")
